...
当前 evaluator 支持 C/C++，其他语言的 ready groups 不是无效结果，而是需要专用 harness 后才能用同口径纳入 covered corpus。AFL edge map 是插桩 GCC 前端执行路径覆盖，不等价于 GCC 源码行/函数覆盖；现已同时保留 AFL edge 与 gcov source coverage 两个质量指标。下一阶段路线：固化 C/C++ covered corpus、提升返回 0 比例和 oracle、补 assembly-scan/diagnostic evaluator，随后实现 Fortran/asm/RTL 等专用 harness；并继续针对 175 个 candidate-only features 的冲突类别设计分流/组合策略。
