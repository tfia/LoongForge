...
论文式 AFL edge 反馈闭环已接入 GroupLLM：`PYTHONPATH=src python3 -m group_llm feedback --output-dir out --instan-output-dir ../instan-llm/out` 会读取 InstanLLM 的 AFL maps，计算 group 的新增 union edge，并把 reward 分配给 source features。`prepare` 会自动读取 `out/afl-feedback/feature-afl-rewards.jsonl`，在下一轮候选采样中优先选择高 reward feature。第一版全局 reward 加权导致跨 target/language/test-mode 混组，真实小批 rejected 率为 91.67%；加入 LoongArch C/C++ AFL harness 兼容池、required target arch hard gate 和 test-mode hard gate 后，真实小批降到 25.00% rejected，9 个新 ready groups 经 InstanLLM 全部 AFL covered，将 union edge 从 261,917 提升到 263,073（+1,156）。阶段评估见 `docs/AFL反馈闭环阶段评估.md`。

当前 evaluator 支持 C/C++，其他语言的 ready groups 不是无效结果，而是需要专用 harness 后才能用同口径纳入 covered corpus。AFL edge map 是插桩 GCC 前端执行路径覆盖，不等价于 GCC 源码行/函数覆盖；现已同时保留 AFL edge feedback 与 gcov source coverage 两个质量指标。下一阶段路线：固化 feedback-guided C/C++ covered corpus、继续降低 GroupLLM rejected 率、提升 InstanLLM oracle 质量，随后实现 assembly-scan、diagnostic、Fortran/asm/RTL 等专用 harness；并继续针对 candidate-only features 的冲突类别设计分流/组合策略。
