...
InstanLLM 阶段已作为 `instan-llm/` 模块接入：读取 `group-llm/out/feature-groups.jsonl` 的 ready records，生成完整测试程序，再通过 `scripts/afl-showmap-gcc.sh` 调用 AFL++ wrapped GCC 前端记录 edge coverage。评估命令默认直接调用 AFL++ 插桩后的 `cc1/cc1plus`，而不是完整 driver 链接执行。2026-08-12 起，InstanLLM evaluation 默认强制 `-Ofast`：清理原有 `-O*` 后在 compiler options 前置 `-Ofast`，以更稳定地压到优化器、向量化、combine、RTL expand/split 等质量风险路径；如需复现实验原始优化级别，可用 `--optimization=preserve`。`-Ofast` 初始重放基线为 269 个 AFL covered、union edge 260,124。五轮有效大批量 feedback loop 后，当前结果为 AFL covered 471、union edge 299,954，累计新增 39,830 edges（+15.31%），ICE-like crash 为 0。阶段报告见顶层 `docs/InstanLLM_阶段覆盖率报告.md`。

源码覆盖率阶段已接入：`scripts/build-gcc-gcov.sh` 构建 coverage 版 GCC 到 `install-gcov/`，`scripts/gcc-source-coverage-replay.py` 读取 `instan-llm/out/evaluations.jsonl` 中 260 个 covered C/C++ corpus 并用 gcov 重放统计 GCC 源码树覆盖。当前结果：源码行覆盖 298,606/909,439（32.83%），函数覆盖 38,216/95,267（40.11%），分支覆盖 224,427/828,708（27.08%）；重放返回 0 为 104/260，非零退出 156/260，常见原因为生成测例包含当前 compiler-only sysroot 不具备的外部头文件。源码覆盖详细报告在 `out/source-coverage/instanllm-covered/gcc-source-coverage-report.md`。

论文式 AFL edge 反馈闭环已接入 GroupLLM：`PYTHONPATH=src python3 -m group_llm feedback --output-dir out --instan-output-dir ../instan-llm/out` 会读取 InstanLLM 的 AFL maps，计算 group 的新增 union edge，并把 reward 分配给 source features。`prepare` 会自动读取 `out/afl-feedback/feature-afl-rewards.jsonl`，在下一轮候选采样中优先选择高 reward feature。reward 公式是 `new_edges_sum + 25.0 * novel_group_count + 0.001 * edge_entries_sum`，只作为采样排序加分；target architecture、ABI、optimization/test-mode、language/frontend/harness compatibility 仍是 hard gate，reward 再高也不能绕过。2026-08-12 长程前修复了 provider empty response 和批量 timeout：GroupLLM/InstanLLM prompt 都要求直接输出 JSON，默认 `max_tokens` 提到 32,000，witness 截断为 1,800；`scripts/run-afl-feedback-loop.py` 对 GroupLLM 与 InstanLLM 均采用逐 group 独立超时。08-12 两轮有效大批量迭代后达到 covered 345、union edge 288,889；08-13 更换 provider 并提高并发后又执行三轮，每轮 48 candidates，GroupLLM ready 分别为 46、44、43，新增 covered 分别为 44、40、42，edge 增量分别为 +7,292、+2,228、+1,545。三轮含补跑总 wall time 约 3.04 小时，单轮约 0.95-1.08 小时；第 3 轮有 1 个 provider JSON 截断导致的 parse_error。阶段评估见 `docs/AFL反馈闭环阶段评估.md`。

当前 evaluator 支持 C/C++，其他语言的 ready groups 不是无效结果，而是需要专用 harness 后才能用同口径纳入 covered corpus。AFL edge map 是插桩 GCC 前端执行路径覆盖，不等价于 GCC 源码行/函数覆盖；现已同时保留 AFL edge feedback 与 gcov source coverage 两个质量指标。ICE-like crash 的处理口径是：`ICE_EXIT_CODE=4` 先标记为 `evaluation_status="ice"`，随后必须用相同 source/options 复现，并对比已有 bug PoC、`source_bug_ids` 和 stderr signature；只有不能被已知 PoC/signature 覆盖的样例才作为新问题候选。截至 2026-08-13 当前 `-Ofast` 评估没有 ICE：`covered=471`、`skipped_not_ready=27`、`ice=0`。下一阶段路线：不要无限长程；建议每次 1-3 个大轮次为单位执行，固化当前 471 个 covered corpus，继续降低 InstanLLM not-ready/rejected、提升 oracle 质量，随后实现 assembly-scan、diagnostic、Fortran/asm/RTL 等专用 harness；并继续针对 candidate-only features 的冲突类别设计分流/组合策略。
